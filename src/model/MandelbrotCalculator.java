...
/**
 * The MandelbrotCalculator class contains methods which establish the Mandelbrot set. The calcMandelbrotSet method below iterates
 * over X,Y positions and establishes for specified parameter values a 2-D array containing for each [y][x] pixel an iteration
 * value that may be mapped to colour and used to visualise the Mandelbrot set. The calcMandelbrotSet method iteratively calls for
 * each X,Y coordinate, the calcMandel method (also included here) which establishes the iteration value for a particular X,Y
 * coordinate.
 *
 * Example usage -- To obtain a 800x800 2-D array of Mandelbrot set values for the initial parameter values
 *
 * MandelbrotCalculator mandelCalc = new MandelbrotCalculator(); int[][] madelbrotData = mandelCalc.calcMandelbrotSet(800, 800,
 * MandelbrotCalculator.INITIAL_MIN_REAL, MandelbrotCalculator.INITIAL_MAX_REAL, MandelbrotCalculator.INITIAL_MIN_IMAGINARY,
...
public class MandelbrotCalculator {
    ...
    /**
     * Method which calculates the number of iterations over which Z_n+1 = Z_n^2 + C can be applied for Z starting at the origin
     * and a specific constant C (given by its Real and cImaginary components). If the square of the absolute value of Z is still
     * inside the defined squaredRadius after maxIterations then we stop iterating and return maxIterations, implicitly assuming
     * that Z will never escape the radius for the given setting of the constant C. In this case, the value of C is treated as
     * being part of the Mandelbrot set, i.e. the set of starting constants C for which the value of Z remains bounded within the
     * complex plane under iteration.
     *
     ...
     * @param maxIterations the maximum number of iterations over which to iterate the equation until assuming Z will remain
     *            bounded.
     * @param radiusSquared the squared of the radius to use when determining whether Z escaped the circle in the complex plain or
     *            remained bounded. The value used is commonly 4.0.
     * @return the number iterations for the value of Z to grow outside of the bounding radius, or maxIterations if it never
     *         escaped.
     */
     ...
     * @param minImaginary the lower imaginary bound for the complex constant C (equivalent to lower bound Y value in Mandelbrot
     *            set)
     * @param maxIterations the maximum number of iterations to iterate the complex formula
     * @param radiusSquared the squared of the radius to use when determining whether Z escaped the circle in the complex plain or
     *            remained bounded.
     * @return the 2-D integer array mandelbrotData[yResolution][xResolution] containing the for each [y][x] pixel the number of
     *         iterations needed until Z escaped the bounding radius, or maxIterations otherwise.
     */
    public static int[][] calcMandelbrotSet(int xResolution, int yResolution, double minReal, double maxReal, double minImaginary, double maxImaginary, int maxIterations, double radiusSquared) {
        ...
        double realStep = realRange / xResolution;
